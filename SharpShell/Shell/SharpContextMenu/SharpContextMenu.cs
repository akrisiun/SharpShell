...
namespace SharpShell.SharpContextMenu
{
    ...
    public abstract class SharpContextMenu : ShellExtInitServer, IContextMenu3
    {
        ...
        HResult IContextMenu2.QueryContextMenu(IntPtr hMenu, uint indexMenu, int idCmdFirst, int idCmdLast, CMF uFlags)
        {
            return (this as IContextMenu).QueryContextMenu(hMenu, indexMenu, idCmdFirst, idCmdLast, uFlags);
        }

        HResult IContextMenu3.QueryContextMenu(IntPtr hMenu, uint indexMenu, int idCmdFirst, int idCmdLast, CMF uFlags)
        {
            var menu = this as IContextMenu;
            return menu == null ? HResult.E_NOTIMPL : menu.QueryContextMenu(hMenu, indexMenu, idCmdFirst, idCmdLast, uFlags);
        }
        ...
        //int IContextMenuSharp2.InvokeCommand(ref CMINVOKECOMMANDINFO pici)
        //{
        //    return ((IContextMenu)this).InvokeCommand(pici);
        //}

        int IContextMenu3.InvokeCommand(IntPtr pici) // ref CMINVOKECOMMANDINFO pici)
        {
            return ((IContextMenu)this).InvokeCommand(pici);
        }
        ...
        HResult IContextMenu2.GetCommandString(int idcmd, GCS uflags, int reserved, StringBuilder commandstring, int cch)
        {
            return (this as IContextMenu).GetCommandString(idcmd, uflags, reserved, commandstring, cch);
        }

        //IContextMenu3.
        public HResult GetCommandString(int idcmd, GCS uflags, int reserved, StringBuilder commandstring, int cch)
        {
            return (this as IContextMenu).GetCommandString(idcmd, uflags, reserved, commandstring, cch);
        }
        ...
        HResult IContextMenu2.HandleMenuMsg(uint uMsg, IntPtr wParam, IntPtr lParam)
        {
            //  Always delegate to the IContextMenu3 version.
            IntPtr result = IntPtr.Zero;
            return (this as IContextMenu3).HandleMenuMsg2(uMsg, wParam, lParam, ref result);
        }

        HResult IContextMenu3.HandleMenuMsg(uint uMsg, IntPtr wParam, IntPtr lParam)
        {
            //  Always delegate to the IContextMenu3 version.
            IntPtr result = IntPtr.Zero;
            return ((IContextMenu3)this).HandleMenuMsg2(uMsg, wParam, lParam, ref result);
        }
        ...
        /// </returns>
        /// <exception cref="System.NotImplementedException"></exception>
        HResult IContextMenu3.HandleMenuMsg2(uint uMsg, IntPtr wParam, IntPtr lParam,
               ref IntPtr plResult)
        {
